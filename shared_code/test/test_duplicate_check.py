...
from shared_code.duplicate_check import ensure_table_exists, store_id, check_duplicate, get_table_service_client
...
class TestStoreID:
    ...
    def test_store_id_success(self, MockEnsureTableExists, MockTableServiceClient):
        ...
        MockTableServiceClient.return_value.get_table_client.return_value = mock_table_client

        result = store_id("some_id", "some_context", MockTableServiceClient.return_value)

        MockEnsureTableExists.assert_called_once_with("some_context", MockTableServiceClient.return_value)
        mock_table_client.create_entity.assert_called_once_with(TableEntity(PartitionKey="messages", RowKey="some_id"))
    ...
    def test_store_id_calls_ensure_table_exists(self, MockTableServiceClient):
        ...
        with patch('shared_code.duplicate_check.ensure_table_exists') as MockEnsureTableExists:
            store_id("some_id", "some_context", MockTableServiceClient.return_value)
    ...
    @patch('shared_code.duplicate_check.TableServiceClient')
    @patch('shared_code.duplicate_check.ensure_table_exists')
    def test_get_table_client_fails(self, MockEnsureTableExists, MockTableServiceClient):
        MockTableServiceClient.return_value.get_table_client.side_effect = Exception("Get table client failed")
...
class TestCheckDuplicate:
    ...
    @patch('shared_code.duplicate_check.TableServiceClient')
    @patch('shared_code.duplicate_check.ensure_table_exists')
    def test_check_duplicate_entity_exists(self, MockEnsureTableExists, MockTableServiceClient):
        mock_table_client = Mock()
        MockTableServiceClient.return_value.get_table_client.return_value = mock_table_client

        result = check_duplicate("some_id", "some_context", MockTableServiceClient.return_value)
    ...
    @patch('shared_code.duplicate_check.TableServiceClient')
    @patch('shared_code.duplicate_check.ensure_table_exists')
    def test_check_duplicate_entity_not_found(self, MockEnsureTableExists, MockTableServiceClient):
        mock_table_client = Mock()
    ...
    @patch('shared_code.duplicate_check.TableServiceClient')
    @patch('shared_code.duplicate_check.ensure_table_exists')
    def test_check_duplicate_raises_exception(self, MockEnsureTableExists, MockTableServiceClient):
        mock_table_client = Mock()
        mock_table_client.get_entity.side_effect = Exception("Random exception")
        MockTableServiceClient.return_value.get_table_client.return_value = mock_table_client

        with pytest.raises(Exception) as e:
            check_duplicate("some_id", "some_context", MockTableServiceClient.return_value)
...
class TestGetTableServiceClient:
    ...
    @patch('shared_code.duplicate_check.TableServiceClient')
    @patch.dict('shared_code.duplicate_check.os.environ', {'AZURE_STORAGE_CONNECTION_STRING': 'mock_connection_string'})
    def test_initialize_table_service(self, MockTableServiceClient):
        MockTableServiceClient.from_connection_string.return_value = MockTableServiceClient.return_value
