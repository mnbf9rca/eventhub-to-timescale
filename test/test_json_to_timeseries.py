...
class Test_send_to_converter:
    def test_send_to_converter_glow_electricitymeter(
        ...
        test_event, messagebody, topic, publisher = get_test_data(
            "glow_electricitymeter"
        )
        (
            spy_glow_to_timescale,
            spy_homie_to_timescale,
            spy_emon_to_timescale,
        ) = get_spies(mocker)
...
class Test_parse_message:
    ...
    def test_parse_message_calls_send_to_converter_where_extract_topic_errors(
        ...
        mocked_extract_topic = mocker.patch(
            "json_to_timeseries.extract_topic", autospec=True
        )
